"""Shared AWS Rekognition digit-plate reading — used by wagon + loco OCR.

Mirrors `inspection/ocr/rekognition_reader.py` (V4).

Rekognition has no digit allowlist, so every ``LINE`` detection is filtered down
to its digit characters after the call.
...
* **Three-frame sheets** — when the image is a vertical sheet of three plate
  ...
  *per crop*. Blindly concatenating **all** of them would fuse three copies of
  ...
  valid-length reading** — the highest-confidence individual LINE that passes
  the validator, or, failing that, the highest-confidence run of
  vertically-consecutive LINEs that passes it (covering two-row plates on the
  sheet). Only if nothing validates does it fall back to the legacy
  concatenate-everything behaviour, so the caller still gets a best-effort
  string to report.
"""
...
log = get_logger("features.ocr.reader")
...
def _pick_best_valid(
    lines: List[Tuple[float, str, float]], validator: Validator
...
def read_digits(
    ...
    try:
        ...
        log.warning("[OCR] Rekognition DetectText failed: %s", e)
        return "", 0.0
    ...
    for det in detections:
        ...
        if not digits:
            continue
        try:
            top = float(det["Geometry"]["BoundingBox"]["Top"])
        except (KeyError, TypeError, ValueError):
            top = 0.0
        lines.append((top, digits, float(det.get("Confidence", 0.0))))
